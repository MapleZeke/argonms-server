...
public class TelnetClient implements SessionDataModel {
	...
	private long loadBanExpire(Connection con, int accountId) throws SQLException {
		long banExpire = 0;
		try (PreparedStatement ps = con.prepareStatement("SELECT `b`.`accountid` "
			...
			try (ResultSet rs = ps.executeQuery()) {
				...
				while (rs.next()) {
					...
					try (PreparedStatement ips = con.prepareStatement("SELECT MAX(`expiredate`) FROM `infractions` "
	...
	public void authenticate(String pwd) {
		try (Connection con = DatabaseManager.getConnection(DatabaseType.STATE)) {
			try (PreparedStatement ps = con.prepareStatement("SELECT `id`,`password`,`salt`,`gm` FROM `accounts` WHERE `name` = ?")) {
						...
						if (correct) {
							if (gm < UserPrivileges.ADMIN) {
								...
								if (hashUpdate) {
									...
									try (PreparedStatement ups = con.prepareStatement("UPDATE `accounts` SET `password` = ?, `salt` = ? WHERE `name` = ?")) {
